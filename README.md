...
开关之外还有两个**档位**，一键切换边界的宽窄（桌面版在 MCP 区域，命令行版写 `config.json` 的 `permission`）：

| 档位 | 路径 | 命令 | 适用 |
| --- | --- | --- | --- |
| **仅在沙箱**（默认） | 锁在项目目录内 | 受限命令白名单 | 日常改代码 |
| **完全权限** | 整块磁盘 | 不限制 | 信任本次任务时 |

档位管**边界多大**，读/写/执行三个开关管**开不开**，两者叠加。

> ⚠️ **白名单不是安全边界。** 默认白名单里含 `powershell` / `pwsh` / `cmd`，
> 为的是让 Windows 上能跑脚本、构建和测试。但 shell 能调用任何程序，
> 所以沙箱档的真实含义是「**工作目录被锁在项目里**」，而不是「只能跑这几个程序」。
> 需要真正的边界时，用完全权限档并自行承担，或改用虚拟机 / 容器。
...
| 模型名识别 | ✅ opus-5 / gpt-5.5 / gpt-5.6-sol / fable-5 / gemini-3.7-flash 等 |
| 推理档位解析 | ✅ 15 个模型名样本通过 |
| 权限档位切换 | ✅ 沙箱档越界与白名单外命令均被拒 |
| 面板跟随 Arena 主题 | ✅ 浅色 ↔ 深色 |
